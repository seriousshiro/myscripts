...
for ((i = 0; i <= $count_strings; i= $i+1000)); do
    check_code_between_thousand=$(docker exec --user user_name ekd-postgresql psql --dbname $ekd_ca_db_name -c "
            ...
            echo "Проверенно - $i"
    # Перебор по сотне
    if [ "$check_code_between_thousand" != "0" ]; then 
            for ((k = $i; k <= $i+100; k++))
            check_code_between_handred=$(docker exec --user user_name ekd-postgresql psql --dbname $ekd_ca_db_name -c "
            SELECT * FROM table_name 
            ORDER BY created_date 
            OFFSET $k LIMIT 1000;" 1>/dev/null;
            echo "$?")
            if [ $check_code_between_handred != "0" ]; then
                # Перебор по каждой записи из сотни
                for ((g = $k; g <= $k+100; g++))
                do
